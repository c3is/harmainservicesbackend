  ...
  app.post("/service-request", async (req, res) => {
    try {
      ...
      // ✅ Create Service Request (EMAIL FIXED)
      ...
      for (const provider of providers) {
        try {
          ...
          const addonsText =
            serviceReq.serviceAddons?.length > 0
              ? serviceReq.serviceAddons.join(", ")
              : "None";
  ...
  📍 Location: ${formatPublicLocation(serviceReq.customerAddress)}
  📅 Time: ${timeText}
  ...
  Reply YES to accept or NO to decline.`,
            ...
            // ⚠️ If using template, you must update template variables accordingly
  ...
  app.post("/webhook/whatsapp", async (req, res) => {
    try {
      ...
      if (text === "YES") {
        ...
        const addonsText =
          serviceReq.serviceAddons?.filter(Boolean).length > 0
            ? serviceReq.serviceAddons.filter(Boolean).join(", ")
            : "None";
